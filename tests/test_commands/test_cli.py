...
def test_setup_flow_selects_profile_and_model(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("daoyi.cli._select_setup_workflow", fake_select)
    monkeypatch.setattr("daoyi.cli._prompt_model_for_profile", lambda profile: "gpt-5.4")
    monkeypatch.setattr("daoyi.cli._login_provider", fake_login)
...
def test_setup_flow_existing_api_key_profile_can_update_secret(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("daoyi.cli._select_setup_workflow", lambda *args, **kwargs: next(selections))
    monkeypatch.setattr("daoyi.cli._select_from_menu", lambda *args, **kwargs: next(selections))
    monkeypatch.setattr("daoyi.cli._confirm_prompt", lambda *args, **kwargs: True)
    monkeypatch.setattr("daoyi.auth.flows.ApiKeyFlow.run", lambda self: "new-key")
    monkeypatch.setattr("daoyi.cli._prompt_model_for_profile", lambda profile: "gpt-4.1")
...
def test_setup_flow_creates_kimi_profile_with_profile_scoped_key(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("daoyi.cli._select_setup_workflow", lambda *args, **kwargs: next(selections))
    monkeypatch.setattr("daoyi.cli._select_from_menu", lambda *args, **kwargs: next(selections))
    monkeypatch.setattr("daoyi.cli._text_prompt", lambda *args, **kwargs: next(prompts))
    monkeypatch.setattr("daoyi.auth.flows.ApiKeyFlow.run", lambda self: "sk-kimi-test")
...
def test_dangerously_skip_permissions_passes_full_auto_to_run_repl(monkeypatch):
    ...
    monkeypatch.setattr("daoyi.ui.app.run_repl", fake_run_repl)
...
def test_task_worker_flag_routes_to_run_task_worker(monkeypatch):
    ...
    monkeypatch.setattr("daoyi.ui.app.run_task_worker", fake_run_task_worker)
...
def test_dry_run_uses_preview_builder_and_skips_repl(monkeypatch):
    ...
    monkeypatch.setattr("daoyi.cli._build_dry_run_preview", fake_build_dry_run_preview)
    monkeypatch.setattr("daoyi.ui.app.run_repl", fake_run_repl)
...
def test_dry_run_rejects_continue_resume(monkeypatch):
    runner = CliRunner()
    monkeypatch.setattr("daoyi.cli._build_dry_run_preview", lambda **kwargs: {"mode": "dry-run"})
...
def test_build_dry_run_preview_classifies_slash_command_and_flags_bad_mcp(monkeypatch, tmp_path: Path):
    settings = Settings(
        api_key="sk-test",
        mcp_servers={
            "broken": McpStdioServerConfig(command="definitely-not-a-real-command-daoyi"),
        },
    )
    ...
    monkeypatch.setattr("daoyi.config.load_settings", lambda: settings)
    monkeypatch.setattr(
        "daoyi.api.provider.detect_provider",
        lambda settings: types.SimpleNamespace(name="anthropic"),
    )
    monkeypatch.setattr("daoyi.api.provider.auth_status", lambda settings: "configured")
    monkeypatch.setattr("daoyi.plugins.load_plugins", lambda settings, cwd: [])
    monkeypatch.setattr("daoyi.skills.load_skill_registry", lambda cwd, settings=None: _FakeSkillRegistry())
    monkeypatch.setattr("daoyi.prompts.context.build_runtime_system_prompt", lambda *args, **kwargs: "preview prompt")
    monkeypatch.setattr("daoyi.ui.runtime._resolve_api_client_from_settings", lambda settings: object())
...
def test_build_dry_run_preview_sets_blocked_when_model_prompt_lacks_auth(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("daoyi.api.provider.auth_status", lambda settings: "missing")
    monkeypatch.setattr("daoyi.plugins.load_plugins", lambda settings, cwd: [])
    monkeypatch.setattr("daoyi.skills.load_skill_registry", lambda cwd, settings=None: _FakeSkillRegistry())
    monkeypatch.setattr("daoyi.prompts.context.build_runtime_system_prompt", lambda *args, **kwargs: "preview prompt")
    ...
    monkeypatch.setattr("daoyi.ui.runtime._resolve_api_client_from_settings", fake_resolve_api_client)
...
def test_build_dry_run_preview_recommends_matching_skills_and_tools(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("daoyi.plugins.load_plugins", lambda settings, cwd: [])
    monkeypatch.setattr("daoyi.skills.load_skill_registry", lambda cwd, settings=None: _FakeSkillRegistry())
    monkeypatch.setattr("daoyi.tools.create_default_tool_registry", lambda: _FakeToolRegistry())
    monkeypatch.setattr("daoyi.prompts.context.build_runtime_system_prompt", lambda *args, **kwargs: "preview prompt")
    monkeypatch.setattr("daoyi.ui.runtime._resolve_api_client_from_settings", lambda settings: object())
...
def test_autopilot_run_next_cli(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("daoyi.autopilot.RepoAutopilotStore", FakeStore)
