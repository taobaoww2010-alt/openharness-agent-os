...
from daoyi.coordinator.agent_definitions import (
    AGENT_COLORS,
    EFFORT_LEVELS,
    ISOLATION_MODES,
    MEMORY_SCOPES,
...
from daoyi.plugins.schemas import PluginManifest
from daoyi.plugins.types import LoadedPlugin, PluginCommandDefinition
from daoyi.skills.loader import _parse_skill_metadata
...
def get_project_plugins_dir(cwd: str | Path) -> Path:
    ...
    path = Path(cwd).resolve() / ".daoyi" / "plugins"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _load_plugin_hooks(path: Path) -> dict[str, list]:
    ...
    if not path.exists():
        return {}
    from daoyi.hooks.schemas import (
        AgentHookDefinition,
...
def _load_plugin_mcp(path: Path) -> dict[str, object]:
    ...
    if not path.exists():
        return {}
    from daoyi.mcp.types import McpJsonConfig
...
def _load_plugin_tools(path: Path, manifest: PluginManifest) -> list:
    ...
    from daoyi.tools.base import BaseTool
