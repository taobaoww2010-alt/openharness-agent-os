...
from daoyi.api.errors import AuthenticationFailure, DaoYiApiError, RateLimitFailure, RequestFailure
...
def _build_codex_headers(token: str, *, session_id: str | None = None) -> dict[str, str]:
    ...
    headers = {
        ...
        "originator": "daoyi",
        "User-Agent": f"daoyi ({platform.system().lower()} {platform.machine() or 'unknown'})",
...
def _translate_status_error(status_code: int, message: str) -> DaoYiApiError:
...
class CodexApiClient:
    ...
    @staticmethod
    def _translate_error(exc: Exception) -> DaoYiApiError:
        if isinstance(exc, DaoYiApiError):
            return exc
        if isinstance(exc, httpx.HTTPStatusError):
