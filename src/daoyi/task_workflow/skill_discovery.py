...
from daoyi.llm.classifier import tier_for_skill
from daoyi.skills.loader import get_user_skills_dir
from daoyi.tools.skill_executor_tool import parse_skill_md
...
class SkillMatcher:
        ...
        for skill_dir in search_dirs:
            ...
            for child in skill_dir.iterdir():
                ...
                if parsed and parsed.name not in skills:
                    tier = tier_for_skill(parsed.name)
                    skills[parsed.name] = {
                        "name": parsed.name,
    ...
    def find_skills(self, user_intent: str, limit: int = 5) -> list[dict[str, Any]]:
        ...
        words = re.findall(r'\w+', intent_lower)
        expanded = set(words)
        for w in words:
            if any(ord(c) > 127 for c in w):
                expanded.update(w)
        intent_words = expanded
