...
from daoyi.api.codex_client import _build_codex_headers, _resolve_codex_url
from daoyi.api.openai_client import _normalize_openai_base_url
    ...
    async def _generate_with_openai(self, arguments: ImageGenerationToolInput, config: dict[str, object]) -> list[str]:
        ...
        if not api_key:
            raise RuntimeError(
                ...
                "or DAOYI_IMAGE_GENERATION_API_KEY, or choose provider='codex'."
            )
    ...
    async def _generate_with_codex(
        ...
        if not auth_token:
            raise RuntimeError(
                ...
                "or use provider='openai' with DAOYI_IMAGE_GENERATION_API_KEY."
            )
