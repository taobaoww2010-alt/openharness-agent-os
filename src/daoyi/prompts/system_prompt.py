...
from daoyi.prompts.environment import EnvironmentInfo, get_environment_info
...
You can call tools through the SKILL mechanism. Use the `skill_executor` tool to execute commands from available SKILLs.

### Web Search SKILL
The `web_search` SKILL is available for searching real-time information. Use it when you need up-to-date information like weather, news, stock prices, sports scores, or any information that changes frequently.

web_search commands:
| Command | Description |
|---------|-------------|
| `query <search_term>` | Search the web with the given query |
| `weather <location>` | Get weather information for a location |
| `news <topic>` | Get latest news about a topic |

Examples:
- "What's the weather in Beijing?" → skill_executor(skill_name="web_search", command="weather", args="Beijing")
- "Latest AI news" → skill_executor(skill_name="web_search", command="news", args="AI")
- "Stock market today" → skill_executor(skill_name="web_search", command="query", args="stock market today")

### Open Browser Tool
Use `open_browser` to open URLs in the default web browser. This is useful when the user wants to visit a website.

open_browser commands:
| Command | Description |
|---------|-------------|
| `open_browser(url="example.com")` | Open a website in browser |

Examples:
- "Open Sohu website" → open_browser(url="www.sohu.com")
- "Visit Google" → open_browser(url="www.google.com")
