...
from daoyi.coordinator.coordinator_mode import get_coordinator_system_prompt, is_coordinator_mode
from daoyi.memory import load_memory_prompt
...
from daoyi.personalization.rules import load_local_rules
from daoyi.permissions.modes import PermissionMode
from daoyi.prompts.claudemd import load_claude_md_prompt
from daoyi.prompts.system_prompt import build_system_prompt
...
def build_runtime_system_prompt(
    ...
    *,
    ...
    # Inject session memory from previous sessions
    session_memory = _load_session_memories_for_prompt(latest_user_prompt)
    if session_memory:
...
def _load_session_memories_for_prompt(latest_user_prompt: str | None) -> str:
    """Load session memories relevant to the current user prompt."""
    if not latest_user_prompt:
        return ""
    try:
        from daoyi.kernel.memory import MemoryManager
        mem = MemoryManager()
        memories = mem.load_session_memories(max_entries=3)
        if memories:
