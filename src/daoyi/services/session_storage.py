...
from daoyi.config.paths import get_sessions_dir
...
_MAX_SESSION_FILES = 50  # max named session snapshots per project before pruning
...
def save_session_snapshot(
    *,
    ...
    # Prune old session files to keep directory bounded
    _prune_old_sessions(session_dir)

    return latest_path


def _prune_old_sessions(session_dir: Path) -> None:
    """Delete oldest session-*.json files beyond *max_files*."""
    try:
        files = sorted(session_dir.glob("session-*.json"), key=lambda p: p.stat().st_mtime, reverse=True)
        if len(files) > _MAX_SESSION_FILES:
            for stale in files[_MAX_SESSION_FILES:]:
