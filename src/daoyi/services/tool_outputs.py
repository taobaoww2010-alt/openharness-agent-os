...
def _read_positive_int_env(name: str, default: int, *, minimum: int = 1) -> int:
    daoyi_name = "DAOYI_" + name[len("OPENHARNESS_"):]
    raw = (os.environ.get(daoyi_name) or os.environ.get(name) or "").strip()
    if not raw:
        return default
    try:
        return max(minimum, int(raw))
    except ValueError:
        log.warning("Ignoring invalid %s|%s=%r", daoyi_name, name, raw)
        return default
