...
from daoyi.memory.usage import find_stale_memory_candidates
from daoyi.services.autodream.backup import create_memory_backup, diff_memory_dirs
...
_CHILD_ENV = "DAOYI_AUTODREAM_CHILD"
...
async def start_dream_now(
    *,
    ...
    env = {
        _CHILD_ENV: "1",
        "DAOYI_AUTODREAM_MEMORY_DIR": str(resolved_memory_dir),
        "OPENHARNESS_AUTODREAM_MEMORY_DIR": str(resolved_memory_dir),
        "DAOYI_CONFIG_DIR": str(Path.home() / ".daoyi"),
        "OPENHARNESS_CONFIG_DIR": str(Path.home() / ".daoyi"),
        "DAOYI_PROFILE": settings.active_profile,
    ...
    try:
        ...
        if runner_module == "daoyi" and settings.provider != "anthropic_claude":
