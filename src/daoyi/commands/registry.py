...
from daoyi.autopilot import RepoAutopilotStore
...
from daoyi.bridge.types import WorkSecret
...
from daoyi.config.settings import Settings, display_model_setting, load_settings, save_settings
...
from daoyi.output_styles import load_output_styles
...
from daoyi.plugins.installer import install_plugin_from_path, uninstall_plugin
...
from daoyi.services.autodream import (
    diff_memory_dirs,
    format_memory_diff,
...
from daoyi.services.memory_extract import extract_memories_from_turn
...
if TYPE_CHECKING:
    from daoyi.config.settings import ProviderProfile
    from daoyi.state import AppStateStore
...
class CommandContext:
    ...
    process_table: Any | None = None  # daoyi.kernel.process.ProcessTable
...
def create_default_command_registry(
    ...
    async def _version_handler(_: str, context: CommandContext) -> CommandResult:
        del context
        try:
            version = importlib.metadata.version("daoyi")
    ...
    async def _dream_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        app_label = backend.label if backend is not None else "daoyi project memory"
        runner_module = "ohmo" if backend is not None and "ohmo" in backend.label.lower() else "daoyi"
        if action == "status":
    ...
    async def _login_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(message="Stored API key in ~/.daoyi/settings.json")
    ...
    async def _theme_handler(args: str, context: CommandContext) -> CommandResult:
        from daoyi.themes import list_themes, load_theme
    ...
    async def _keybindings_handler(_: str, context: CommandContext) -> CommandResult:
        from daoyi.keybindings import get_keybindings_path, load_keybindings
    ...
    async def _voice_handler(args: str, context: CommandContext) -> CommandResult:
        from daoyi.voice import extract_keyterms, inspect_voice_capabilities
    ...
    async def _privacy_settings_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        lines = [
            ...
            "- storage: local files under ~/.daoyi and project .daoyi",
        ]
        return CommandResult(message="\n".join(lines))
    ...
    async def _workflow_handler(args: str, context: CommandContext) -> CommandResult:
        from daoyi.task_workflow import get_workflow_registry
        ...
        if subcmd == "ps":
            pt = context.process_table
            if pt is None:
                return CommandResult(message="Process table not available.")
            return CommandResult(message="── Agent OS Process Table ──\n" + pt.format_ps())

        if subcmd == "kill" and len(parts) >= 2:
            pt = context.process_table
            if pt is None:
                return CommandResult(message="Process table not available.")
            try:
                pid = int(parts[1])
                from daoyi.kernel.process import Signal
                ok = pt.send_signal(pid, Signal.SIGTERM)
                return CommandResult(
                    message=f"Sent SIGTERM to pid={pid}" if ok else f"pid={pid} not found."
                )
            except ValueError:
                return CommandResult(message=f"Invalid pid: {parts[1]}")

        if subcmd == "stats" and len(parts) >= 2:
            pt = context.process_table
            if pt is None:
                return CommandResult(message="Process table not available.")
            try:
                ...
                proc = pt.get(pid)
                if proc is None:
                    return CommandResult(message=f"pid={pid} not found.")
                import json
                mem = getattr(proc, "mem_stats", None)
                if mem:
                    return CommandResult(message=json.dumps(mem, indent=2, ensure_ascii=False))
                return CommandResult(message=f"pid={pid}: no memory stats available.")
            except ValueError:
                return CommandResult(message=f"Invalid pid: {parts[1]}")

        if subcmd == "list":
            workflows = wf_registry.list()
            if not workflows:
                return CommandResult(message="No workflows registered.")
            lines = ["Registered workflows:"]
            for wf in workflows:
                phase_info = ", ".join(p.name for p in wf.phases)
                lines.append(
                    f"  {wf.id}: {len(wf.phases)} phases [{phase_info}] "
                    f"(used {wf.use_count}x, triggers: {wf.trigger_patterns})"
                )
            return CommandResult(message="\n".join(lines))

        if subcmd == "show" and len(parts) >= 2:
            wf = wf_registry.get(parts[1])
            if wf is None:
                return CommandResult(message=f"Workflow '{parts[1]}' not found.")
            import json
            return CommandResult(
                message=json.dumps(wf.to_dict(), indent=2, ensure_ascii=False)
            )

        if subcmd == "delete" and len(parts) >= 2:
            ok = wf_registry.delete(parts[1])
            return CommandResult(
                message=f"Deleted workflow '{parts[1]}'." if ok else f"Workflow '{parts[1]}' not found."
            )

        if subcmd == "reset":
            for wf in wf_registry.list():
                wf_registry.delete(wf.id)
            for bw in BUILTIN_WORKFLOWS:
                wf_registry.save(bw)
            return CommandResult(
                message=f"Reset workflows to {len(BUILTIN_WORKFLOWS)} built-in templates."
            )
        ...
        if subcmd == "snapshot":
            ...
            snapshots = sm.list()
            if not snapshots:
                return CommandResult(message="No snapshots available.")
            lines = ["Snapshots (most recent first):"]
            for s in snapshots[:20]:
                lines.append(
                    f"  {s.snapshot_id:<30} pid={s.pid} workflow={s.workflow_id} "
                    f"phase={s.phase_name}"
                )
            return CommandResult(message="\n".join(lines))

        if subcmd == "restore" and len(parts) >= 2:
            return CommandResult(
                message="Restore not available from CLI. "
                        "Use the snapshot ID with a workflow execution context."
            )

        return CommandResult(
            message="Usage: /workflow list|show <id>|delete <id>|reset|ps|kill <pid>|stats <pid>|snapshot|restore <id>"
        )

    registry.register(
        SlashCommand("workflow", "List, inspect, and manage task workflow templates", _workflow_handler)
    )
