"""Path resolution for DaoYi Agent OS configuration and data directories.

Follows XDG-like conventions with ~/.daoyi/ as the default base directory.
"""
...
def get_config_dir() -> Path:
    ...
    Resolution order:
    1. DAOYI_CONFIG_DIR environment variable
    2. OPENHARNESS_CONFIG_DIR environment variable (backward compat)
    3. ~/.daoyi/
    """
    env_dir = os.environ.get("DAOYI_CONFIG_DIR") or os.environ.get("OPENHARNESS_CONFIG_DIR")
    if env_dir:
...
def get_config_file_path() -> Path:
    """Return the path to the main settings file (~/.daoyi/settings.json)."""
...
def get_data_dir() -> Path:
    ...
    Resolution order:
    1. DAOYI_DATA_DIR environment variable
    2. OPENHARNESS_DATA_DIR environment variable (backward compat)
    3. ~/.daoyi/data/
    """
    env_dir = os.environ.get("DAOYI_DATA_DIR") or os.environ.get("OPENHARNESS_DATA_DIR")
    if env_dir:
...
def get_logs_dir() -> Path:
    ...
    Resolution order:
    1. DAOYI_LOGS_DIR environment variable
    2. OPENHARNESS_LOGS_DIR environment variable (backward compat)
    3. ~/.daoyi/logs/
    """
    env_dir = os.environ.get("DAOYI_LOGS_DIR") or os.environ.get("OPENHARNESS_LOGS_DIR")
    if env_dir:
...
def get_project_config_dir(cwd: str | Path) -> Path:
    """Return the per-project .daoyi directory."""
    project_dir = Path(cwd).resolve() / ".daoyi"
