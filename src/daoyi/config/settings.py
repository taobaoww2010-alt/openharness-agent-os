...
1. CLI arguments
2. Environment variables (ANTHROPIC_API_KEY, DAOYI_MODEL / OPENHARNESS_MODEL, etc.)
3. Config file (~/.daoyi/settings.json)
4. Defaults
"""
...
from daoyi.config.schema import ChannelConfigs
from daoyi.hooks.schemas import HookDefinition
from daoyi.mcp.types import McpServerConfig
...
class DockerSandboxSettings(BaseModel):
    ...
    image: str = "daoyi-sandbox:latest"
...
def default_provider_profiles() -> dict[str, ProviderProfile]:
    ...
    return {
        ...
        "cpp": ProviderProfile(
            label="C++ Local LLM Engine",
            provider="cpp",
            api_format="anthropic",
            auth_source="cpp_local",
...
def auth_source_env_var_candidates(auth_source: str) -> tuple[str, ...]:
    ...
    def _ak(name: str) -> tuple[str, ...]:
        return (f"DAOYI_{name}", f"OPENHARNESS_{name}", name)

    mapping = {
        "anthropic_api_key": _ak("ANTHROPIC_API_KEY"),
        "openai_api_key": _ak("OPENAI_API_KEY"),
        "dashscope_api_key": _ak("DASHSCOPE_API_KEY"),
        "moonshot_api_key": _ak("MOONSHOT_API_KEY"),
        "gemini_api_key": _ak("GEMINI_API_KEY"),
        "minimax_api_key": _ak("MINIMAX_API_KEY"),
        "nvidia_api_key": _ak("NVIDIA_API_KEY"),
        "modelscope_api_key": _ak("MODELSCOPE_API_KEY"),
    }
...
class ImageGenerationConfig(BaseModel):
    ...
    def from_env(cls) -> "ImageGenerationConfig":
        ...
        def _img_env(name: str, default: str = "") -> str:
            val = os.environ.get(f"DAOYI_IMAGE_GENERATION_{name}") or os.environ.get(f"OPENHARNESS_IMAGE_GENERATION_{name}") or default
            return val.strip()

        return cls(
            provider=_img_env("PROVIDER", "auto") or "auto",
            model=_img_env("MODEL", "gpt-image-2") or "gpt-image-2",
            api_key=_img_env("API_KEY"),
            base_url=_img_env("BASE_URL"),
            codex_model=_img_env("CODEX_MODEL", "gpt-5.4") or "gpt-5.4",
            codex_base_url=_img_env("CODEX_BASE_URL"),
        )
...
class VisionModelConfig(BaseModel):
    ...
    def from_env(cls) -> "VisionModelConfig":
        ...
        def _vis_env(name: str) -> str:
            return (os.environ.get(f"DAOYI_VISION_{name}") or os.environ.get(f"OPENHARNESS_VISION_{name}") or "").strip()
        return cls(
            model=_vis_env("MODEL"),
            api_key=_vis_env("API_KEY"),
            base_url=_vis_env("BASE_URL"),
        )
...
class Settings(BaseModel):
    ...
    project_skill_dirs: list[str] = Field(
        default_factory=lambda: [".daoyi/skills", ".agents/skills", ".claude/skills"]
    )
    ...
    def resolve_profile(self, name: str | None = None) -> tuple[str, ProviderProfile]:
        ...
        profile_name = (name or self.active_profile or os.environ.get("DAOYI_PROFILE") or os.environ.get("OPENHARNESS_PROFILE") or "").strip() or "claude-api"
    ...
    def sync_active_profile_from_flat_fields(self) -> Settings:
        ...
        """
        profile_name, profile = self.resolve_profile()
        profile_from_env = bool(os.environ.get("DAOYI_PROFILE") or os.environ.get("OPENHARNESS_PROFILE"))
    ...
    def resolve_api_key(self) -> str:
        ...
        raise ValueError(
            "No API key found. Set a DAOYI_* (or OPENHARNESS_*) provider API key "
            "(preferred) or the matching native provider environment variable, "
            "or configure api_key in ~/.daoyi/settings.json"
        )
    ...
    def resolve_auth(self) -> ResolvedAuth:
        ...
        if auth_source in {"codex_subscription", "claude_subscription"}:
            ...
            from daoyi.auth.storage import load_external_binding
        ...
        from daoyi.auth.storage import load_credential
...
def _apply_env_overrides(settings: Settings) -> Settings:
    ...
    explicitly configure the corresponding field.  ``DAOYI_*`` env vars
    always override (explicit user intent); ``OPENHARNESS_*`` kept for
    ...
    # --- model ---
    daoyi_model = os.environ.get("DAOYI_MODEL") or os.environ.get("OPENHARNESS_MODEL")
    if daoyi_model:
        updates["model"] = strip_ansi_escape_sequences(daoyi_model)
    ...
    # --- base_url ---
    daoyi_base = os.environ.get("DAOYI_BASE_URL") or os.environ.get("OPENHARNESS_BASE_URL")
    if daoyi_base:
        updates["base_url"] = daoyi_base
    ...
    max_tokens = os.environ.get("DAOYI_MAX_TOKENS") or os.environ.get("OPENHARNESS_MAX_TOKENS")
    if max_tokens:
        updates["max_tokens"] = int(max_tokens)

    timeout = os.environ.get("DAOYI_TIMEOUT") or os.environ.get("OPENHARNESS_TIMEOUT")
    if timeout:
        updates["timeout"] = float(timeout)

    max_turns = os.environ.get("DAOYI_MAX_TURNS") or os.environ.get("OPENHARNESS_MAX_TURNS")
    if max_turns:
        updates["max_turns"] = int(max_turns)

    context_window_tokens = os.environ.get("DAOYI_CONTEXT_WINDOW_TOKENS") or os.environ.get("OPENHARNESS_CONTEXT_WINDOW_TOKENS")
    if context_window_tokens:
        updates["context_window_tokens"] = int(context_window_tokens)

    auto_compact_threshold_tokens = os.environ.get("DAOYI_AUTO_COMPACT_THRESHOLD_TOKENS") or os.environ.get("OPENHARNESS_AUTO_COMPACT_THRESHOLD_TOKENS")
    if auto_compact_threshold_tokens:
        updates["auto_compact_threshold_tokens"] = int(auto_compact_threshold_tokens)

    provider = os.environ.get("DAOYI_PROVIDER") or os.environ.get("OPENHARNESS_PROVIDER")
    api_format = os.environ.get("DAOYI_API_FORMAT") or os.environ.get("OPENHARNESS_API_FORMAT")
    ...
    sandbox_enabled = os.environ.get("DAOYI_SANDBOX_ENABLED") or os.environ.get("OPENHARNESS_SANDBOX_ENABLED")
    sandbox_fail = os.environ.get("DAOYI_SANDBOX_FAIL_IF_UNAVAILABLE") or os.environ.get("OPENHARNESS_SANDBOX_FAIL_IF_UNAVAILABLE")
    sandbox_backend = os.environ.get("DAOYI_SANDBOX_BACKEND") or os.environ.get("OPENHARNESS_SANDBOX_BACKEND")
    sandbox_docker_image = os.environ.get("DAOYI_SANDBOX_DOCKER_IMAGE") or os.environ.get("OPENHARNESS_SANDBOX_DOCKER_IMAGE")
    ...
    # Channel env vars: auto-enable Feishu if credentials are set
    feishu_app_id = os.environ.get("DAOYI_FEISHU_APP_ID") or os.environ.get("OPENHARNESS_FEISHU_APP_ID")
    feishu_app_secret = os.environ.get("DAOYI_FEISHU_APP_SECRET") or os.environ.get("OPENHARNESS_FEISHU_APP_SECRET")
    if feishu_app_id or feishu_app_secret:
        channels = settings.channels.model_copy(
            update={
                "feishu": settings.channels.feishu.model_copy(
                    update={
                        "enabled": True,
                        "app_id": feishu_app_id or settings.channels.feishu.app_id,
                        "app_secret": feishu_app_secret or settings.channels.feishu.app_secret,
                    }
                )
            }
        )
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    if config_path is None:
        from daoyi.config.paths import get_config_file_path
    ...
    if config_path.exists():
        ...
        env_profile = os.environ.get("DAOYI_PROFILE") or os.environ.get("OPENHARNESS_PROFILE")
        if env_profile:
