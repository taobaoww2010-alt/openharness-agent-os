...
from daoyi.engine.cost_tracker import CostTracker
from daoyi.coordinator.coordinator_mode import get_coordinator_user_context
from daoyi.engine.messages import ConversationMessage, TextBlock, ToolResultBlock, sanitize_conversation_messages
from daoyi.engine.query import AskUserPrompt, PermissionPrompt, QueryContext, remember_user_goal, run_query
from daoyi.engine.stream_events import AssistantTurnComplete, StreamEvent
from daoyi.config.settings import Settings
from daoyi.hooks import HookEvent, HookExecutor
from daoyi.permissions.checker import PermissionChecker
from daoyi.services.autodream.service import schedule_auto_dream
...
class QueryEngine:
    ...
    def _prepare_session_memory(self) -> None:
        ...
        if not self._settings.memory.enabled:
            return
        from daoyi.services.session_memory import prepare_session_memory_metadata
    ...
    async def _update_session_memory(self) -> None:
        ...
        if not self._settings.memory.enabled:
            return
        from daoyi.services.session_memory import update_session_memory_file
