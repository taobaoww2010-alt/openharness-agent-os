...
TEAMMATE_COMMAND_ENV_VAR = "DAOYI_TEAMMATE_COMMAND"
...
_TEAMMATE_ENV_VARS = [
    ...
    "REQUESTS_CA_BUNDLE",
    "CURL_CA_BUNDLE",
    # --- DaoYi-native provider settings (DAOYI_ > OPENHARNESS_ compat) -------
...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``DAOYI_TEAMMATE_COMMAND`` / ``OPENHARNESS_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. The current Python interpreter running the ``daoyi`` module.
       This keeps spawned teammates on the same venv/source tree as the
       leader process.
    3. The ``daoyi`` entry-point on PATH (installed package fallback).
    """
    override = os.environ.get(TEAMMATE_COMMAND_ENV_VAR) or os.environ.get("OPENHARNESS_TEAMMATE_COMMAND")
    if override:
        return override
    ...
    entry_point = shutil.which("daoyi")
    if entry_point:
        return entry_point
    return "python"
...
def build_inherited_cli_flags(
    *,
    ...
    # --- Model override ----------------------------------------------------
    # "inherit" means use the parent's model via the DAOYI_MODEL / OPENHARNESS_MODEL env var.
...
def build_inherited_env_vars() -> dict[str, str]:
    ...
    Always includes ``DAOYI_AGENT_TEAMS=1`` / ``OPENHARNESS_AGENT_TEAMS=1`` plus any provider/proxy
