...
from daoyi.swarm.mailbox import get_team_dir
from daoyi.swarm.types import BackendType
...
async def _kill_orphaned_teammate_panes(team_name: str) -> None:
    ...
    """
    from daoyi.swarm.registry import get_backend_registry
    from daoyi.swarm.spawn_utils import is_inside_tmux
    from daoyi.swarm.types import is_pane_backend
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.daoyi/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.daoyi/teams/``, sorted by name."""
        base = Path.home() / ".daoyi" / "teams"
        if not base.exists():
            return []
