...
from daoyi.utils.helpers import safe_filename
...
_fh = logging.FileHandler("/tmp/daoyi-feishu.log", mode="a")
...
class FeishuChannel(BaseChannel):
    ...
    def _on_message_sync(self, data: "P2ImMessageReceiveV1") -> None:  # noqa: F821
        ...
        """
        logger.info("Feishu _on_message_sync called with data: %s", type(data).__name__)
        if self._loop and self._loop.is_running():
            asyncio.run_coroutine_threadsafe(self._on_message(data), self._loop)
        else:
            logger.warning("Feishu _on_message_sync: loop not available or not running")
    ...
    async def _on_message(self, data: "P2ImMessageReceiveV1") -> None:  # noqa: F821
        ...
        try:
            ...
            sender = event.sender
            logger.info(
                "Feishu _on_message: msg_type=%s chat_type=%s sender_type=%s content=%.50s",
                message.message_type, message.chat_type, sender.sender_type,
                message.content or "",
            )
            ...
            # Add reaction (best-effort, never blocks message processing)
            try:
                await self._add_reaction(message_id, self.config.react_emoji)
            except Exception as e:
                logger.debug("Feishu: failed to add reaction: %s", e)
