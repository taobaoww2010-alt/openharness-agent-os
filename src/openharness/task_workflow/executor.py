...
from openharness.kernel import HAS_CPP_CORE
from openharness.kernel.memory import MemoryManager
from openharness.kernel.process import Process, ProcessState, ProcessTable, Signal
from openharness.kernel.snapshot import SnapshotManager
from openharness.kernel.syscall import SyscallTable, Syscall
from openharness.permissions.checker import PermissionChecker
from openharness.tools.base import ToolRegistry, ToolExecutionContext

if TYPE_CHECKING:
    from openharness.task_workflow.models import TaskWorkflow

if HAS_CPP_CORE:
    import _openharness as _CPP
...
class WorkflowExecutor:
    ...
    def __init__(
        ...
        from openharness.llm.small_model import SmallModelClient
    ...
    async def execute(
        ...
        *,
        ...
        for phase_idx, phase in enumerate(workflow.phases):
            ...
            try:
                from openharness.task_workflow.skill_context_injector import get_skill_context_injector
                from openharness.api.client import ToolDefinition

                injector = get_skill_context_injector()
                skill_schema = injector.get_skill_command_schema("cli-anything-gimp")
                if skill_schema:
                    scoped_registry._tools.append(ToolDefinition.from_dict(skill_schema))
            except Exception:
                pass  # SKILL tool injection is optional
    ...
    def _classify_with_cpp(self, user_input: str) -> str | None:
        """Fast regex-based classification via C++ ClassifierInterface."""
        if not HAS_CPP_CORE:
            return None
        try:
            ...
            if conf < 0.7 or intent == 0:  # UNKNOWN
                return None
            return self._INTENT_MAP.get(intent)
        except Exception:
            return None

    def pre_classify(self, user_input: str) -> str | None:
        """Classify user intent.

        Priority: C++ Classifier (regex, deterministic) → SmallModelClient (LLM) → None

        Returns ``"tool"``, ``"chat"``, ``"code"``, ``"search"``,
        ``"file_ops"``, or ``"code_review"`` — or *None* if unavailable.
        """
        cpp_result = self._classify_with_cpp(user_input)
        if cpp_result is not None and cpp_result != "chat":
            return cpp_result
